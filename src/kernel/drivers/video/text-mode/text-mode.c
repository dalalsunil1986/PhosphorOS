...
static inline void textmode_clearcolor(uint8_t color) {
    curx = 0; cury = 0;
...
static inline void textmode_clearlinecolor(int l, uint8_t color) {
    curx = 0;
...
static inline uint8_t textmode_getfgc(int x, int y) {
    return (uint8_t)((textmode_vbuf[x + y * TMW] >> 8) & 0x0F);
}

static inline uint8_t textmode_getbgc(int x, int y) {
...
static inline void textmode_setfgc(int x, int y, uint8_t c) {
    textmode_vbuf[x + y * TMW] = textmode_getchar(x, y) + TMC(c, textmode_getbgc(x, y));
}

static inline void textmode_setbgc(int x, int y, uint8_t c) {
